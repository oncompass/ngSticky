  ...
  module.directive('sticky', ['$window', '$timeout', function($window, $timeout) {
      return {
        ...
        link: function linkFn($scope, $elem, $attrs) {
          ...
          var stickyParent = $attrs.stickyParent || '';

          // Find scrollbar
          var scrollbar = deriveScrollingViewport ($elem, stickyParent);
           ...
           * if stickyParent is specified tries to listen to that element scroll event
           */
          function deriveScrollingViewport(stickyNode, stickyParent) {
            if (stickyParent && angular.element(stickyParent).length > 0) {
              return angular.element(stickyParent).get(0);
            }
